    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          ['Sources', bySource.length, '#38BDF8'],
      ...
      <div className="rounded-xl border overflow-hidden" style={{ borderColor: 'rgba(124,58,237,.18)' }}>
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y" style={{ borderColor: 'rgba(124,58,237,.08)' }}>
            {rows.map(row => (
              <tr key={row.id}>
                ...
                <td className="px-4 py-3">
                  <span className="font-mono text-[10px] px-2 py-1 rounded" style={{ color: row.status === 'active' ? '#10B981' : '#F59E0B', background: row.status === 'active' ? 'rgba(16,185,129,.12)' : 'rgba(245,158,11,.12)' }}>
                    {row.status ?? 'unknown'}
                  </span>
                </td>
            ...
            {rows.length === 0 && (
              <tr><td colSpan={4} className="px-4 py-12 text-center font-mono text-xs" style={{ color: 'rgba(155,142,196,.5)' }}>ยังไม่มี subscribers</td></tr>
            )}
          </tbody>
        </table>
      </div>
