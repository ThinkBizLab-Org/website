      ...
      <section className="grid grid-cols-1 xl:grid-cols-2 gap-5">
        <Panel title="Waiting LINE approval" subtitle="drafts that need your review">
          <Rows empty="ไม่มีงานรอ approve">
            {waiting.map(topic => <TopicRow key={topic.id} topic={topic} />)}
          </Rows>
        </Panel>
...
function TopicRow({ topic }: { topic: FactoryTopic }) {
  const color = statusColor[topic.status] ?? '#9B8EC4'
  return (
    <Link href={topic.articleId ? `/admin/articles/${topic.articleId}` : '/admin/calendar'} className="flex items-center gap-3 px-4 py-3 hover:bg-white/5">
